...
class A_Star:
    ...
    def heuristic(self, successors):
        ...
        '''Implements Manhatten Distance and path cost in order to determine the most
        promising state in a list of successors.

        The Manhatten Distance of each successor state is determined by the sum of
        each pegs' distance from the center of the board. Path cost is then added to the
        Manhatten Distance of a state, which is how deep down the tree the current state
        is. After the sum is calculated, the successor states are sorted by least value.

        :param successors: The list of successors to be evaluated
        :return: The list of sorted successors by least value
        '''
    ...
    def path_cost(self, successor):

        '''Takes in a successor state and calculates the cost of a path.

        :param successor: The current successor to have its path cost evaluated
        :return: The path cost of the current successor state
        '''
